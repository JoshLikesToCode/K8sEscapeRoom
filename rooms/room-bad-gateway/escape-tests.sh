#!/usr/bin/env bash
# escape-tests.sh - Validate that room-bad-gateway has been ESCAPED (fixed)
#
# Success criteria:
...
NAMESPACE="escape-room-bad-gateway"
POD_LABEL="app=escape-app"
SERVICE_NAME="escape-service"
INGRESS_NAME="escape-ingress"

echo "=== Testing room-bad-gateway (escaped/fixed state) ==="
echo ""
