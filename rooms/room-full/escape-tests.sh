#!/usr/bin/env bash
# escape-tests.sh - Validate that room-full has been ESCAPED (fixed)
#
# Success criteria:
...
echo "=== Testing room-full (escaped/fixed state) ==="
echo ""

# Verify pod exists
if ! kubectl get pod "$POD_NAME" -n "$NAMESPACE" &>/dev/null; then
    echo -e "${RED}No pod named '$POD_NAME' found in namespace $NAMESPACE${NC}"
    echo "Make sure you've applied the room first: make room-apply ROOM=room-full"
    exit 1
fi
