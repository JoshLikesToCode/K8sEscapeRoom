...
kubectl get svc -n escape-room-ingress-misroute -o name
# Output: service/escape-service

kubectl get ingress escape-ingress -n escape-room-ingress-misroute -o jsonpath='{.spec.rules[0].http.paths[0].backend.service.name}'
...
kubectl run test-curl --rm -it --image=curlimages/curl --restart=Never \
  -n escape-room-ingress-misroute -- curl -s http://escape-service
...
```bash
kubectl get ingress escape-ingress -n escape-room-ingress-misroute -o yaml | \
...
kubectl run test-ingress --rm -it --image=curlimages/curl --restart=Never \
  -n escape-room-ingress-misroute -- \
