...
```bash
kubectl describe deployment escape-app -n escape-room-access-denied
```

Look for `Service Account` in the output. Then check what permissions that ServiceAccount has:
```bash
kubectl auth can-i list pods --as=system:serviceaccount:escape-room-access-denied:escape-sa -n escape-room-access-denied
```
