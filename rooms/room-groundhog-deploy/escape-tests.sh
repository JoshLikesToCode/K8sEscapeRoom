#!/usr/bin/env bash
# escape-tests.sh - Validate that room-groundhog-deploy has been ESCAPED (fixed)
#
# Success criteria:
...
NAMESPACE="escape-room-groundhog-deploy"
POD_LABEL="app=escape-app"
DEPLOYMENT_NAME="escape-app"

echo "=== Testing room-groundhog-deploy (escaped/fixed state) ==="
echo ""
...
if [ -z "$POD_NAME" ]; then
    ...
    echo "Make sure you've applied the room first: make room-apply ROOM=room-groundhog-deploy"
    exit 1
fi
